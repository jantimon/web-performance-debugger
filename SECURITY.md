...
- The only network traffic is the page you name and its sourcemap fetches. Those fetches run under a
  bounded policy: only http(s) URLs, no private or loopback host reached from a public page, a size cap
  per script and map, and a per-run time budget. Nothing else is fetched.
