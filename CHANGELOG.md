...
- [#219](https://github.com/jantimon/web-performance-debugger/pull/219) [`c25e640`](https://github.com/jantimon/web-performance-debugger/commit/c25e6408a7daf68976765ac451372df18b038517) - Keep the timing series for repeated named measures in capture order and expose it
...
- [#218](https://github.com/jantimon/web-performance-debugger/pull/218) [`f230801`](https://github.com/jantimon/web-performance-debugger/commit/f23080149e9fb5402439de431fd100844d223a1e) - Expose run and driver-step timing samples through `query span --format json|toon`.
...
- 0d3899d: `cpu-diff --fail-on-regression` no longer fires the JS-self gate when BOTH recordings' `jsSelfMs` sit
...
- b2c9e46: The `--breakdown` reconciling bar's js-slice footer now describes the tiled bar it sits under: js is
...
- 7e0c7df: Driver steps now record a navigation classification. Each step carries `beforeUrl`/`afterUrl` (its
...
- 44c29aa: A heavy `--deep` capture now fails fast with a named error the moment the trace is captured, above a
...
- 981abb9: **Breaking:** the CPU headline is now true JS self-time, and old recordings must be re-recorded.
...
- c623d2e: Firefox dogfood fixes plus a cross-engine `query spans` and CPU-attribution fix:
...
- 8ec4a00: Faster Firefox capture turnaround: the shutdown-dump poll now confirms the dump in a few tight reads instead of a fixed ~750 ms floor (the dump is already complete when `browser.close()` resolves), so every `--target firefox` run finishes sooner.
...
- 8c83400: Add **run groups**: record several capture modes of one workload as siblings under a
